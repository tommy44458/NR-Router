...
class Mesh():
    ...
    def create_hub (self):
        for i in range(self.hubs1_length):
            ...
            self.hubs3[i] = Hub(real_x=self.grids3[i//3, 0].real_x, real_y=hubs3_y, type=0, hub_index=i)
        elif i%3==1:
            ...
            if thub_grid_x1!=thub_grid_x2:
                if abs((thub_grid_x1*self.Tile_Unit+self.block2_shift[0])-(thub_tile_x1*self.Control_pad_unit+block1_shift[0]))<950:# and abs(thub_real_x1-(thub_tile_x1*Control_pad_unit+block1_shift[0]))<abs(thub_real_x2-(thub_tile_x2*Control_pad_unit+block1_shift[0])):
                    thub_grid_x1+=1
                    thub_grid_x2+=1
                ...
                elif abs((thub_grid_x2*self.Tile_Unit+self.block2_shift[0])-(thub_tile_x2*self.Control_pad_unit+block1_shift[0]))<950:
                    thub_grid_x1-=1
                    thub_grid_x2-=1
            ...
            if abs((thub_grid_x1*self.Tile_Unit+self.block2_shift[0])-(thub_tile_x1*self.Control_pad_unit+block1_shift[0]))<1000 or abs(((thub_grid_x1+1)*self.Tile_Unit+self.block2_shift[0])-((thub_tile_x1+1)*self.Control_pad_unit+block1_shift[0]))<1000:
                #print(i)
                ...
                self.hubs1[i] = Hub(real_x=self.block1_shift[0]+(i//3)*2540+1160+(i%3-1)*220, real_y=hubs1_y, type=1, hub_index=i)
                self.hubs3[i] = Hub(real_x=self.block1_shift[0]+(i//3)*2540+1160+(i%3-1)*220, real_y=hubs3_y, type=1, hub_index=i)
                i+=1
                self.hubs1[i] = Hub(real_x=self.block1_shift[0]+(i//3)*2540+1160+(i%3-1)*220, real_y=hubs1_y, type=1, hub_index=i)
                self.hubs3[i] = Hub(real_x=self.block1_shift[0]+(i//3)*2540+1160+(i%3-1)*220, real_y=hubs3_y, type=1, hub_index=i)
            else:
                self.hubs1[i] = Hub(real_x=thub_grid_x1*self.Tile_Unit+self.block2_shift[0], real_y=hubs1_y, type=1, hub_index=i)
                self.hubs3[i] = Hub(real_x=thub_grid_x1*self.Tile_Unit+self.block2_shift[0], real_y=hubs3_y, type=1, hub_index=i)
                i+=1
                self.hubs1[i] = Hub(real_x=(thub_grid_x1+1)*self.Tile_Unit+self.block2_shift[0], real_y=hubs1_y, type=1, hub_index=i)
                self.hubs3[i] = Hub(real_x=(thub_grid_x1+1)*self.Tile_Unit+self.block2_shift[0], real_y=hubs3_y, type=1, hub_index=i)
                
    def _create_neighbor_electrodes(self, x,y,dir): 
        if dir[0]==1 and self.girds2[i-1][j-1].type>0:   #012
                                                    #3 4
                                                    #567
            self.girds2[i][j].neighbor_electrode.append([self.girds2[i-1][j-1].electrode_index,0,-1,-1]) # index dir self.girds2[i-1][j-1]
        if dir[1]==1 and self.girds2[i][j-1].type>0:
            #avoid wire over boundary
            if self.girds2[i][j].real_x < self.electrodes[self.girds2[i][j-1].electrode_index].boundary_L + 150 or self.girds2[i][j].real_x > self.electrodes[self.girds2[i][j-1].electrode_index].boundary_R - 150:
                pass
            else:
                self.girds2[i][j].neighbor_electrode.append([self.girds2[i][j-1].electrode_index,1,0,-1])
        if dir[2]==1 and self.girds2[i+1][j-1].type>0:
            self.girds2[i][j].neighbor_electrode.append([self.girds2[i+1][j-1].electrode_index,2,1,-1])
        if dir[3]==1 and self.girds2[i-1][j].type>0:
            self.girds2[i][j].neighbor_electrode.append([self.girds2[i-1][j].electrode_index,3,-1,0])
        if dir[4]==1 and self.girds2[i+1][j].type>0:
            self.girds2[i][j].neighbor_electrode.append([self.girds2[i+1][j].electrode_index,4,1,0])
        if dir[5]==1 and self.girds2[i-1][j+1].type>0:
            self.girds2[i][j].neighbor_electrode.append([self.girds2[i-1][j+1].electrode_index,5,-1,1])
        if dir[6]==1 and self.girds2[i][j+1].type>0:
            #avoid wire over boundary
            if self.girds2[i][j].real_x < self.electrodes[self.girds2[i][j+1].electrode_index].boundary_L + 150 or self.girds2[i][j].real_x > self.electrodes[self.girds2[i][j+1].electrode_index].boundary_R - 150:
                pass
            else:
                self.girds2[i][j].neighbor_electrode.append([self.girds2[i][j+1].electrode_index,6,0,1])
        if dir[7]==1 and self.girds2[i+1][j+1].type>0:
            self.girds2[i][j].neighbor_electrode.append([self.girds2[i+1][j+1].electrode_index,7,1,1])
        ...
        for i in range (self.grids2_length[0]):
            for j in range(self.grids2_length[1]):
                if self.grids2[i][j].safe_distance==1:
                    if i!=0 and len(grids2[i-1][j].neighbor_electrode)==0 and self.grids2[i-1][j].safe_distance==0:
    ...
    def create_hubs_connection(self, hubs, hubs_length, block2_n, tile_n, grids, tiles):
        for i in range(hubs_length):
            left = int((hubs[i].real_x-block1_shift[0]) // self.Tile_Unit)
            right = int((hubs[i].real_x-block1_shift[0]) // self.Tile_Unit+1)
            if i%3==0:
                if (hubs[i].real_x - self.girds2[left][block2_n].real_x) > (self.Tile_Unit/2):
                    near = right
                else:
                    near = left
                if grids[i//3][tile_n].special==False:
                    self.girds2[near][block2_n].neighbor.append([hubs[i], 1, 1944])
                    hubs[i].neighbor.append([grids[i//3][tile_n], 1, 1944])
            elif i%3==1:
                if (self.girds2[right][block2_n].real_x - hubs[i].real_x) > (250/2):
                    self.girds2[left][block2_n].neighbor.append([hubs[i], 1, 1944])
                else:
                    self.girds2[right][block2_n].neighbor.append([hubs[i], 1, 1944])
                hubs[i].neighbor.append([tiles[i//3][tile_n], 1, 2694])
            else:
                if (hubs[i].real_x - self.girds2[left][block2_n].real_x) > (250/2):
                    self.girds2[right][block2_n].neighbor.append([hubs[i], 1, 1944])
                else:
                    self.girds2[left][block2_n].neighbor.append([hubs[i], 1, 1944])
                hubs[i].neighbor.append([tiles[i//3][tile_n], 1, 2694])
            ...
            if electrode[2] == shape[i]:
                    ...
                    if x1==x2: ## liking grid and elector
                        if y1<y2:#downward
                            ...
                            for k in range(E_grid_y2 - E_grid_y1 + 1):
                                self.girds2[E_grid_x1][E_grid_y1+k].electrode_index=self.num_electrode #elector into grid
                                self.girds2[E_grid_x1][E_grid_y1+k].type+=1
                                self.girds2[E_grid_x1][E_grid_y1+k].electrode_x = x1 
                                self.girds2[E_grid_x1][E_grid_y1+k].electrode_y = self.girds2[E_grid_x1][E_grid_y1+k].real_y
                                # if (self.girds2[E_grid_x1][E_grid_y1-1].electrode_index == self.girds2[E_grid_x1][E_grid_y1].electrode_index) or k!=0:
                                #     self.girds2[E_grid_x1][E_grid_y1+k].electrode_x = x1 
                                #     self.girds2[E_grid_x1][E_grid_y1+k].electrode_y = self.girds2[E_grid_x1][E_grid_y1+k].real_y
                                # else:
                            ...
                            for k in range(E_grid_y1-E_grid_y2+1):
                                self.girds2[E_grid_x1][E_grid_y1-k].electrode_index=self.num_electrode
                                self.girds2[E_grid_x1][E_grid_y1-k].type+=1
                                self.girds2[E_grid_x1][E_grid_y1-k].electrode_x = x1 
                                self.girds2[E_grid_x1][E_grid_y1-k].electrode_y = self.girds2[E_grid_x1][E_grid_y1-k].real_y
                                # if (self.girds2[E_grid_x1][E_grid_y1+1].electrode_index == self.girds2[E_grid_x1][E_grid_y1].electrode_index) or k!=0:
                        ...
                        if x1<x2:#----------------->
                            ...
                            for k in range(E_grid_x2-E_grid_x1+1):
                                self.girds2[E_grid_x1+k][E_grid_y1].electrode_index=self.num_electrode
                                self.girds2[E_grid_x1+k][E_grid_y1].type+=1
                                self.girds2[E_grid_x1+k][E_grid_y1].electrode_x = self.girds2[E_grid_x1+k][E_grid_y1].real_x
                                self.girds2[E_grid_x1+k][E_grid_y1].electrode_y = y1
                                # if (self.girds2[E_grid_x1-1][E_grid_y1].electrode_index==self.girds2[E_grid_x1][E_grid_y1].electrode_index) or k!=0:
                                #     self.girds2[E_grid_x1+k][E_grid_y1].electrode_x = self.girds2[E_grid_x1+k][E_grid_y1].real_x
                                #     self.girds2[E_grid_x1+k][E_grid_y1].electrode_y = y1
                                # else:
                ...
                for j in range(len(electrode_shape_path)-1):
                    ...
                    if x1 != x2 and y1 != y2:
                        ...
                        # self.girds2[E_grid_x2,E_grid_y2].electrode_index=num_electrode
                        # self.girds2[E_grid_x2,E_grid_y2].type+=1
                        # self.girds2[E_grid_x2,E_grid_y2].electrode_x = (x1+x2)/2
                        # self.girds2[E_grid_x2,E_grid_y2].electrode_y = (y1+y2)/2

                        # self.girds2[E_grid_x1,E_grid_y1].electrode_index=num_electrode
                        # self.girds2[E_grid_x1,E_grid_y1].type+=1
                        # self.girds2[E_grid_x1,E_grid_y1].electrode_x = (x1+x2)/2
                        # self.girds2[E_grid_x1,E_grid_y1].electrode_y = (y1+y2)/2

                        # if E_grid_x1!=E_grid_x2 and E_grid_y1!=E_grid_y2:
                        #     self.girds2[max(E_grid_x1,E_grid_x2),max(E_grid_y1,E_grid_y2)].electrode_index=num_electrode
                        #     self.girds2[max(E_grid_x1,E_grid_x2),max(E_grid_y1,E_grid_y2)].type+=1
                        #     self.girds2[max(E_grid_x1,E_grid_x2),max(E_grid_y1,E_grid_y2)].electrode_x = (x1+x2)/2
                        #     self.girds2[max(E_grid_x1,E_grid_x2),max(E_grid_y1,E_grid_y2)].electrode_y = (y1+y2)/2
                        ...
                        if x1>x2:
                            ...
                            if y1>y2:
                                # E_grid_y2+=1
                                # E_grid_x1-=1
                                self.girds2[E_grid_x1,E_grid_y1+1].electrode_index=self.num_electrode
                                self.girds2[E_grid_x1,E_grid_y1+1].type+=1
                                self.girds2[E_grid_x1,E_grid_y1+1].electrode_x = (x1+x2)/2
                                self.girds2[E_grid_x1,E_grid_y1+1].electrode_y = (y1+y2)/2
                                # self.girds2[E_grid_x1,E_grid_y1+1].electrode_y2 = y2

                                self.girds2[E_grid_x2,E_grid_y2+1].electrode_index=self.num_electrode
                                self.girds2[E_grid_x2,E_grid_y2+1].type+=1
                                self.girds2[E_grid_x2,E_grid_y2+1].electrode_x = (x1+x2)/2
                                self.girds2[E_grid_x2,E_grid_y2+1].electrode_y = (y1+y2)/2
                                # self.girds2[E_grid_x2,E_grid_y2].electrode_y2 = y2
                                # self.girds2[E_grid_x1,E_grid_y2].electrode_y2 = y2
                            ...
                            # /
                                ...
                                # E_grid_x2+=1
                                #y1
                                self.girds2[E_grid_x1+1,E_grid_y1+1].electrode_index=self.num_electrode
                                self.girds2[E_grid_x1+1,E_grid_y1+1].type+=1
                                self.girds2[E_grid_x1+1,E_grid_y1+1].electrode_x = (x1+x2)/2
                                self.girds2[E_grid_x1+1,E_grid_y1+1].electrode_y = (y1+y2)/2
                                # self.girds2[E_grid_x1+1,E_grid_y1].electrode_y2 = y1

                                self.girds2[E_grid_x2+1,E_grid_y2+1].electrode_index=self.num_electrode
                                self.girds2[E_grid_x2+1,E_grid_y2+1].type+=1
                                self.girds2[E_grid_x2+1,E_grid_y2+1].electrode_x = (x1+x2)/2
                                self.girds2[E_grid_x2+1,E_grid_y2+1].electrode_y = (y1+y2)/2

                                # self.girds2[E_grid_x2,E_grid_y2+1].electrode_y2 = y1
                                # self.girds2[E_grid_x2+1,E_grid_y1+1].electrode_y2 = y1
                            ...
                            if y1<y2:
                                # x1+1, y2
                                self.girds2[E_grid_x1+1,E_grid_y1].electrode_index=self.num_electrode
                                self.girds2[E_grid_x1+1,E_grid_y1].type+=1
                                self.girds2[E_grid_x1+1,E_grid_y1].electrode_x = (x1+x2)/2
                                self.girds2[E_grid_x1+1,E_grid_y1].electrode_y = (y1+y2)/2
                                # self.girds2[E_grid_x1,E_grid_y1].electrode_y2 = y2

                                self.girds2[E_grid_x2+1,E_grid_y2].electrode_index=self.num_electrode
                                self.girds2[E_grid_x2+1,E_grid_y2].type+=1
                                self.girds2[E_grid_x2+1,E_grid_y2].electrode_x = (x1+x2)/2
                                self.girds2[E_grid_x2+1,E_grid_y2].electrode_y = (y1+y2)/2
                                # self.girds2[E_grid_x2+1,E_grid_y2].electrode_y2 = y2
                                # self.girds2[E_grid_x1+1,E_grid_y2-1].electrode_y2 = y2
                                # print( '4', E_grid_x1,E_grid_y1, E_grid_x2+1,E_grid_y2 )
                            #  /
                            # |
                            else:
                                # x1, y2
                                self.girds2[E_grid_x2,E_grid_y2].electrode_index=self.num_electrode
                                self.girds2[E_grid_x2,E_grid_y2].type+=1
                                self.girds2[E_grid_x2,E_grid_y2].electrode_x = (x1+x2)/2
                                self.girds2[E_grid_x2,E_grid_y2].electrode_y = (y1+y2)/2
                                # self.girds2[E_grid_x2,E_grid_y2].electrode_y2 = y1

                                self.girds2[E_grid_x1,E_grid_y1].electrode_index=self.num_electrode
                                self.girds2[E_grid_x1,E_grid_y1].type+=1
                                self.girds2[E_grid_x1,E_grid_y1].electrode_x = (x1+x2)/2
                                self.girds2[E_grid_x1,E_grid_y1].electrode_y = (y1+y2)/2
                                # self.girds2[E_grid_x1,E_grid_y1].electrode_y2 = y1
                                # self.girds2[E_grid_x2-1,E_grid_y1-1].electrode_y2 = y1
